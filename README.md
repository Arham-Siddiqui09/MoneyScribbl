...
moneyscribbl is an Android personal finance app built with Jetpack Compose. It helps users track income and expenses, manage savings goals, organize spending with folders and limits, review analytics, and scan UPI merchant QR codes to launch payments and log them in the app.
...
moneyscribbl includes a QR-based UPI payment flow that keeps payment tracking simple:

1. Open the `QR` tab or tap `Scan QR` from the home screen.
2. Grant camera permission when prompted.
3. Scan a merchant UPI QR code.
4. moneyscribbl reads the merchant details from the QR and shows the payee name, UPI ID, note, and amount when available.
...
8. moneyscribbl lists detected UPI apps installed on the device.
9. After the user chooses a UPI app, moneyscribbl opens it and records the payment in transaction history as an expense entry.
...
- The QR payment flow opens supported UPI apps and records the action from moneyscribbl's side, but final payment confirmation still happens inside the selected UPI app.
